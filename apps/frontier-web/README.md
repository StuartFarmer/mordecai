...
The frontier example, ported from the original CosmWasm mordecai and
running here on Mordecai: claim tiles, build farms
...
HyperDHT, so the page talks JSON to an `@mordecai/gateway` (any node operator
can run one) and signs every transaction locally with Ed25519
(`@noble/curves`) over `@mordecai/protocol`'s canonical signing bytes. The
...
pnpm --filter @mordecai/example-frontier-web build   # this app → dist/
...
```sh
pnpm --filter @mordecai/example-frontier-web dev   # Vite, proxies /api → :8787
```
...
| --------------------------------- | --------------------------------------------- |
| `frontier.pysc` (mordsl)          | `compiler/examples/frontier.pysc` (mordecaic) |

# Summary

[Introduction](introduction.md)
[Quickstart](quickstart.md)

# Architecture

- [The Two Planes](architecture/overview.md)
- [Three Kinds of State](architecture/state.md)
- [Networking: Holepunch](architecture/networking.md)
- [The Settlement Chain](architecture/chain.md)
- [Consensus](architecture/consensus.md)
- [The Contract VM](architecture/vm.md)
- [Wire Format & Protocol](architecture/protocol.md)
- [Threat Model](architecture/threat-model.md)

# Building P2P Applications

- [What a P2P App Is (and Isn't)](apps/what-is-a-p2p-app.md)
- [The SDK](apps/sdk.md)
- [Wallets, Identity & Sessions](apps/identity.md)
- [Publishing & Installing Apps](apps/publishing.md)

# Smart Contracts

- [The Contract Model](contracts/model.md)
- [Tutorial: A Wager Contract in the DSL](contracts/tutorial.md)
- [DSL Language Reference](contracts/dsl-reference.md)
- [Rust Contracts & the ABI](contracts/rust-abi.md)

# Examples

- [Chess: A Complete P2P App](examples/chess.md)
- [The Marketplace Contract](examples/marketplace.md)
- [Land: State Maps & Config](examples/land.md)

# Reference

- [CLI Tools](reference/cli.md)
- [RPC API](reference/rpc.md)
- [Repository Layout & Building](reference/repo.md)

---

[Roadmap & Limitations](roadmap.md)
